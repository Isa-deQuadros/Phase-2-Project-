...
function GameCard({
  gameCardDataFromContainer,
  renderGame,
  descriptionToggle,
  descriptionState,
  newReviewFormFunction,
}) {
  const [musicState, setMusicState] = useState(true);
  const [audio, setAudio] = useState(
    new Audio(gameCardDataFromContainer.score)
  );
  ...
  function increaseLikes() {
    ...
    fetch(
      `http://localhost:3002/gameCard/${gameCardDataFromContainer.id}`,
      config
    )
      .then((res) => res.json())
  ...
  function DecreaseLikes() {
    const config = {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        dislikes: gameCardDataFromContainer.dislikes + 1,
      }),
    };
    fetch(
      `http://localhost:3002/gameCard/${gameCardDataFromContainer.id}`,
    ...
    <Container>
      <div className="card">
        <img
          onClick={() =>
            renderGame(
              gameCardDataFromContainer.name,
              gameCardDataFromContainer.reviews,
              gameCardDataFromContainer.trailer,
              gameCardDataFromContainer.description
            )
          }
          className="card_image"
          src={gameCardDataFromContainer.imageURL}
          alt="game_image"
        />
        ...
        <p classList="description">
          {" "}
          {descriptionToggle &&
          gameCardDataFromContainer.description === descriptionState
            ? gameCardDataFromContainer.description
            : null}
        </p>
        ...
        <div className="review-form">
          {reviewForm ? (
            <NewReviewForm newReviewFormFunction={newReviewFormFunction} />
          ) : null}
        </div>
      </div>
    </Container>
